...
type Leadership = PortfolioData["leadership"][0]

function emptyLeadership(): Leadership {
  return {
...
export default function LeadershipEditor() {
  ...
  async function handleSave() {
    setSaving(true)
    try {
      await savePortfolioSection("leadership", { items })
      setSaved(true)
  ...
  function updateItem(id: string, updates: Partial<Leadership>) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-mono text-2xl text-white mb-1">Leadership</h1>
          <p className="text-white/40 text-sm font-mono">Manage leadership and community roles</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => setItems([...items, emptyLeadership()])}
